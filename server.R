...
#  output$Go <- eventReactive(input$Go,  
    {
      ...
      withProgress(message = "Running Sig2Lead", value = 0, {
        ...
        #output$check_connection <- renderUI({
        #  p("No response from server.  Please check connection and press enter to continue.")
        #})
      ...
      #1. Query iLINCS for user input gene
        if (input$Signature == "Define Target Gene"){
          ...
          #shinyjs::showElement("check_connection")
        
          #shiny::validate(need((!is.null(define_knockdown(input$gene_knockdown))), "Something went wrong."))
          ...
          #validate(need(ConDisconSignatures(input$gene_knockdown), "Something went wrong."))
          ...
          #########################################################################################
          # Added 08/13/2022
          #ConDisconSignatures(connected_sigs)
        
          #ConDisconSignatures <<- ConDisconSignatures(connected_sigs)
          ...
          if(input$ConOrDiscon_2 == "Inhibit"){
          ...
          else if(input$ConOrDiscon_2 == "Activate"){
          ...
          #output$downloadLINCSUpload <- renderUI({
          #    downloadButton("downloadLINCsUpload", label = "LINCS Signatures Ranked")})
...
#       # test_2 <- list()
#       # test_3 <- "" 
#       # for (i in 1:length(sdfset_add)){
#       #   
#       #   test[[i]] <- sdf2smiles(sdfset_add[i])
#       #   test_2[[i]] <- as(test[[i]], "SMI")
#       #   test_3 <- c(test_3, test_2[[i]])
#       # }
...
#       # test_4[[1]] <- NULL
#       # 
#       # test_5 <- as(test_4, "SMIset")
#       # labels <- cid(test_5)
#       # labels_2 <<- str_sub(labels, end=-2)
#       # 
#       #added_smiles <- as(test_5, "list")
#                             
#       
#       
#       ############# Commented out 1/19/20201
#       #temp <- list() 
#       
#       #for (i in 1:length(sdfset_add)){
#       #  temp[[i]] <- sdf2smiles(sdfset_add[i])
#       #}
#       
#       #temp2 <- as(temp,"SMIset")
#       #added_smiles <- as.character(test_4)
#       #added_smiles <- added_smiles
#       #added_smiles_a <- str_sub(added_smiles, start=22)
#       
#       #added_smiles_b <- str_sub(added_smiles_a, end=-3)
#       
#       
#       #added_labels <- cid(added_smiles)
#       #added_labels <- labels_2
#       #added_smiles <- unlist(added_smiles)
#       #added_labels <- cid(added_smiles)
#       #added_labels <- cid(added_smiles)
#       #added_labels <- labels_2
#       #
#       #added_smiles_2 <- as.character(added_smiles[1:length(added_smiles)])
#       
#       #added_smiles_3 <- cbind(added_labels, added_smiles_b)
#       
#       #added_labels <- cid(added_smiles)
#       #added_labels <- labels_2
#       
...
#       
#       # Added 1/19/2021################3
#       #added_smiles_output <<- as.character(added_smiles)
#       
...
#       ##lsm_smiles_3 <- as.character(lsm_smiles_2[1:length(lsm_smiles_2)])
#       ##lsm_smiles_3b <- unname(lsm_smiles_3)
#       ##sm_smiles_4 <- cbind(lsm_smiles_2_labels, lsm_smiles_3b)
#       ##colnames(lsm_smiles_4) <- c("Compound_ID", "SMILES")
#       
#       display <- display[,1]
#       #colnames(display) <-  c("Compound_ID", "SMILES")
#       #colnames(display) <-  "Compound_ID"
#       
#       ########### End Commented out section 1/19/2021
#       
#       #test2 <<- rbind(added_smiles_3, display)
#       test2 <- c(added_ids, display)
#       
#       #colnames(test2) <- "Compound_ID"
...
#           colnames(df_centroid) <-c("Representative ID", "Cluster")
#           
#           
#           ######### Added
#          
...
#                                                          color = list(background = "white", border = "black", #"aefc6a" 
...
#                                                          color = list(background = "magenta" , border = "black", #background ="#bb84ff" 
#                                                                       highlight = list(background = "caff44", 
#                                                                                        border = "red"))) %>%
...
###############################################################################################################################################
  ...
  #  },
  #  content = function(filename){
  #    write.csv(example_file, filename, row.names=FALSE)
  #  }
  #)
